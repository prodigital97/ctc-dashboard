...
async function updateCssViaEditor() {
  ...
  if (!nonceMatch) {
    ...
    fs.writeFileSync('editor_response.html', html);
    return;
  }
  const editNonce = nonceMatch[1];
  console.log(`[Theme Editor API] Found edit nonce: ${editNonce}`);

  // Load the local style.css content
  const cssPath = path.join(__dirname, '..', 'astra-child-ctc', 'style.css');
  const cssContent = fs.readFileSync(cssPath, 'utf8');

  console.log('[Theme Editor API] 3. Saving updated style.css...');
  ...
  formParams.append('newcontent', cssContent);
  ...
  if (updateText.includes('File edited successfully') || updateText.includes('file-edited-successfully') || updateText.includes('File edited')) {
    console.log('[Theme Editor API] Success! style.css updated successfully on the live site.');
  } else {
    ...
    if (errorMatch) {
      ...
      console.log('Response did not report success. Check editor_result.html for details.');
      fs.writeFileSync('editor_result.html', updateText);
    }
  }
}
