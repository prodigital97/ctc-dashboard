...
console.log(`Loaded ${pool.length} blogs from pool. Found ${targetBlogs.length} target blogs to publish.`);
...
// Build a map of Row -> Category Tags for category resolution
...
// Hardcoded fallbacks if rowToCategoriesMap is empty
...
async function publishPost(item) {
  ...
  // 3. Build Payload (instantly live)
  const payload = {
    title: item.title,
    content: item.bodyHtml, // Already styled by style_converter.js
  ...
  console.log(`[WordPress] Publishing Row ${item.row}: "${item.title}"`);
